  ...
  <v-dialog
    ...
    <template v-slot:activator="{ on }">
      <v-list-item v-on="on">
        ...
        <v-list-item-content>
          <v-list-item-title>{{ $t('aboutDialog.title') }}</v-list-item-title>
        </v-list-item-content>
      </v-list-item>
    </template>
    ...
    <v-card>
      ...
      <v-card-text>
        {{ $t('aboutDialog.introtext') }}
      </v-card-text>

      <v-divider/>

      <v-card-text style="height: 300px;">
        <h2 class="headline text--primary mt-4 mb-2">{{ $t('aboutDialog.releases') }}</h2>
        <div v-if="loading">
